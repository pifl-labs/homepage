...
피피 낱말항해 방침 4파일(ko/ja/en/root)의 광고 조항을 정정했다. ①"배너·전면 광고 표시" 포맷 열거 → "광고가 표시될 수 있습니다"로 일반화 ②존재하지 않는 버전 "1.1.0" → 실제 광고 활성화 버전 **1.0.2**. 배경: 앱이 2026-08-01부터 전면·배너를 플래그 OFF(보상형 힌트만 노출)했는데, ASC 심사 중인 iOS 1.0.2 build 12는 3종 활성이라 **일반화 표현만이 양쪽 빌드에 참**. 보상형 설명(하루 5회·코인 대체·강제 없음)과 IDFA/AAID·ATT 조항은 유지. PR #39 squash 머지(`main@cb287e8`), pr-merge-reviewer 승인.

**수정된 파일:** `src/pages/{ko,ja,en}/apps/pipi-word-voyage/privacy.astro` + `src/pages/apps/pipi-word-voyage/privacy.astro` — 각 광고 조항 1문단 (diff 총 4줄).

### 빌드/검증

- `npm run build` ✅ 113페이지 · dist 4/4에 신카피 반영 실측 · 리포 전체 "1.1.0" 잔존 0
- 라이브 4 URL 확인: ko/ja/en HTTP 200 + 1.0.2 반영 + 구문구 잔존 0, root는 미들웨어 302→/ko/(정상)
- ⚠️ CF 에지 캐시가 배포 직후 수 분간 신/구 혼재 — 라이브 검증은 배포 몇 분 후 재확인이 정답

### 남은 작업

1. ~~en/root §4 Permissions 조항 미래형 문구 잔존~~ → **같은 날 후속 PR #40으로 해소**(`main@e1a81c3`) — ko 제5조·ja 第4条와 동일 의미의 현행 문구(ATT 첫 실행 요청·거부 시 비개인화·기능 제한 없음·Android 광고 ID 재설정)로 교체, 4개 언어 미래형 잔존 0.
